  ...
  var stats = [
    {
      ...
      data: require('../lib/stats/01')(data.income['2017'], new Date().getMonth() + 1)
    },
    {
      title: 'Среден месечен разход по месеци, групиран по разход',
      data: require('../lib/stats/02')(data.spend, ['2011', '2017'])
    },
    {
      title: 'Среден месечен разход, групиран по месеци',
      data: require('../lib/stats/03')(data.spend, ['2011', '2017'])
    },
    {
      title: 'Среден месечен разход до сега',
      data: require('../lib/stats/04')(data.spend, 5, 6)
    },
    {
      title: 'Oбщ разход до сега групиран по вид разход',
      data: require('../lib/stats/02')(data.spend, [])
    }
  ]

  return render({view: view(stats)})
    .then((rendered) =>
      html.prettyPrint(
        '<!DOCTYPE html>\n' + rendered,
        {indent_size: 2}
